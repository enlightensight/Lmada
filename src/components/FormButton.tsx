...
export default function FormButton({ children, disabled = false, loading = false }: FormButtonProps) {
  return (
    <button
      ...
      className="w-full inline-flex items-center justify-center gap-2 px-6 py-4 rounded-xl text-sm font-semibold tracking-wide text-white bg-gradient-to-b from-brand-blue to-sky-500 hover:from-brand-blue-hover hover:to-sky-600 border border-brand-blue/20 shadow-md hover:shadow-lg active:scale-[0.99] transition-all duration-300 disabled:opacity-50 disabled:pointer-events-none select-none cursor-pointer"
    >
      {loading ? (
